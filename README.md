...
### DE Africa Coastlines analysis outputs
Files generated by DE Africa Coastlines are exported to the `data` directory. 

Temporary raster and vector outputs produced by [`dea_coastlines.raster`](dea_coastlines/raster.py) and [`dea_coastlines.vector`](dea_coastlines/vector.py) for each study area grid cell are exported to `data/interim/raster/{unique_analysis_name}/{unique_analysis_name}_{study_area_name}` and `data/interim/vector/{unique_analysis_name}/{unique_analysis_name}_{study_area_name}` respectively.

Once all study area grid cells have been processed, these are combined into continental-scale output vector files using [`dea_coastlines.continental`](dea_coastlines/continental.py). These final outputs are exported to `data/processed/{unique_analysis_name}`.
